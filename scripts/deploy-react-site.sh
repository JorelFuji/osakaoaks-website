...
ssh -p $SSH_PORT $SSH_USER@$SSH_HOST "mkdir -p ~/website-backups && cd $REMOTE_PATH && tar -czf ~/website-backups/$BACKUP_NAME --exclude='*.tar.gz' --exclude='.git' --exclude='node_modules' --exclude='archived-wordpress' ."
